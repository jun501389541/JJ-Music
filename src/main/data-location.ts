...
export function resolveDataDir(input: DataDirInput): DataDirChoice {
  ...
  if (input.pointer) {
    if (input.exists(input.pointer)) return { dir: input.pointer, source: 'pointer', notice: null }
    ...
    return {
      ...
      notice: `上次把数据移到了 ${input.pointer}，该位置现在不可用，本次使用的是${portable ? '程序旁边的数据目录' : '系统应用数据目录'}。`
    }
  }
  if (portable) {
    return {
