...
import { lastPageOf } from '../composables/use-scroll-memory'
...
/*
 * Each link resolves through `lastPageOf`, so clicking 专辑 after reading one
 * album reopens that album instead of dumping the grid back at the top. Paths
...
<template><aside class="sidebar" aria-label="主导航">
  ...
  -->
  <nav><div class="nav-group"><RouterLink v-for="item in browse" :key="item.to" :to="lastPageOf(item.to)" class="nav-item" :draggable="false"><AppIcon :name="item.icon"/><span>{{ item.label }}</span><small v-if="counts[item.to] !== undefined">{{ counts[item.to] }}</small></RouterLink></div>
...
<style scoped>
.sidebar{width:var(--sidebar-width);flex:none;padding:12px 12px 10px 14px;display:flex;flex-direction:column;background:var(--bg-elevated)}/* No `scrollbar-width` here: the global ::-webkit-scrollbar rule is the one the content pane uses, and opting this column into the browser's thin variant made the two scrollbars look different side by side. */nav{flex:1;overflow:auto}/* The rail uses the app-wide scroll-revealed scrollbar (base.css + utils/scroll-reveal.ts); this element used to run its own copy of that logic. */.nav-group{padding:4px 0 14px}.nav-item{position:relative;display:flex;align-items:center;gap:15px;min-height:43px;margin:2px 0;padding:9px 14px;width:100%;color:var(--text-primary);text-decoration:none;background:none;border:0;border-radius:6px;font:inherit;font-size:14px;text-align:left;cursor:pointer}.nav-item span{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.nav-item small{font-size:11px;opacity:.45}.nav-item:hover{background:var(--bg-hover)}.nav-item.router-link-active{background:var(--bg-hover)}.nav-item.router-link-active:before{content:'';position:absolute;left:0;top:13px;bottom:13px;width:3px;background:var(--accent);border-radius:4px}/* 歌单管理 is a row like any other; 新建歌单 rides at its right edge and only
