...
| 发现音乐 | 最近播放、曲库概览 |
| 全局搜索 | 本地结果支持拼音全拼与首字母，输入时有联想提示；在线可搜 QQ、网易云、酷狗、酷我、咪咕 |
...
- **数据目录**：便携版（ZIP）与"程序目录可写"的安装版，把全部数据放在 exe 旁边的 `data/`；程序目录不可写（如装进 `Program Files`）时回退到 `%APPDATA%/jj-music`。Chromium 的缓存随数据目录一起走，不会单独留在 C 盘。首次以便携版启动时会把旧的 `%APPDATA%/jj-music` 复制过来（原目录保留，浏览器缓存等可再生数据不复制）。设置 → App 数据 显示当前目录与来源，并提供「迁移到其它目录」：选完目录后复制并写入 `data-location.json` 指针，需重启生效。迁移过的便携版如果那块盘当天没插，会回到程序旁边的 `data/` 并在设置页说明，不会改到 C 盘去建新库。命令行显式传 `--user-data-dir` 时以它为准（测试与自动化入口依赖这一点）。
...
- **写入位置与「待写入」**：设置 → 标签与文件 决定「写入封面与歌词」落在音频文件标签、同名文件，还是两者都写；格式不能改写时自动退到同名文件并说明原因。播放一首本地歌时联网找到的歌词**不会**自动改你的文件：它先进待写入队列（`library/pending-assets.json`，重启还在），徽标写「（待写入）」，你在这页或歌曲菜单里点「写入」才落盘，也可以整个丢弃。可以改写哪些格式也在这页勾选，这个开关只能收窄、不能放宽应用真会写的格式集合。索引版本因此从 4 升到 5（封面同名文件是新读出来的字段）。
...
- 下载支持断点续传：中断后 `.part` 保留，重试带 `Range` 从已写字节继续。服务器忽略 `Range` 返回整份时会重头下载而不是拼接；`If-Range` 用首次的 `ETag`/`Last-Modified` 判断文件有没有变。同时运行两个任务，每批最多 500 首，单文件最大 1 GB。
- 歌单导入保存本地快照，不自动同步。每次最多读取 5000 首，私密歌单、短链接或平台访问限制可能导致无法完整读取。导入的封面来自平台，抓不到时只提示、不影响歌曲导入。
- 艺术家头像来自各平台的公开接口，只按标签里的歌手名搜索匹配：名字近似但并非同一人时有可能取错，取不到则沿用专辑封面。酷我的艺人接口需要签名，不参与。
