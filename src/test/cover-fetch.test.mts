...
test('没有 picUrl 就一个请求都不发', async () => {
  ...
  assert.equal(await fetchCoverBytes({ picUrl: '' }, get), null)
...
test('下载沿用候选预览已有的尺寸与超时上限（不引入第四个数字）', async () => {
  ...
  await fetchCoverBytes({ picUrl: 'https://p.example.com/a.png' }, get)
  assert.equal(get.calls.length, 1)
  assert.deepEqual(get.calls[0].options, { maxBytes: 8 * 1024 * 1024, timeoutMs: 10_000 })
})

/* ------------------------------------------------------- what counts as art */

test('字节以 Uint8Array 交出，mimeType 用响应头声明的那种', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.png' }, stub('image/png'))
...
test('带参数的 content-type 还是同一张图', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.jpg' }, stub('image/jpeg; charset=binary'))
  assert.equal(cover?.mimeType, 'image/jpeg', `参数没被剥掉的话会得到「${cover?.mimeType}」`)
})

test('image/jpg 归一成 image/jpeg（有的 CDN 就这么写）', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.jpg' }, stub('image/jpg'))
  assert.equal(cover?.mimeType, 'image/jpeg')
})

test('没有 content-type 时退回 jpeg，与改动前一致', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a' }, stub(null))
  assert.equal(cover?.mimeType, 'image/jpeg')
})
...
test('text/html 的响应体被拒，不会当作 JPEG 写进用户的音乐文件夹', async () => {
  ...
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.jpg' }, stub('text/html', portal))
  assert.equal(cover, null, 'HTML 被当成封面收下了')
})

test('写入端不认识的图片类型同样拒掉（GIF 不在 sidecar 名单里）', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.gif' }, stub('image/gif'))
  assert.equal(cover, null, 'GIF 会在落盘时被改名成 .jpg，内容却还是 GIF')
})

test('空响应体不算一张封面', async () => {
  const cover = await fetchCoverBytes({ picUrl: 'https://p.example.com/a.png' }, stub('image/png', Buffer.alloc(0)))
...
test('网络失败返回 null，不把异常抛回正在写歌词的那次调用', async () => {
  ...
  assert.equal(await fetchCoverBytes({ picUrl: 'https://127.0.0.1/a.png' }, get), null)
})
