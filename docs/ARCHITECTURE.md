...
该边界用于隔离崩溃和死循环，不是权限沙箱。脚本在 Node 全局上下文执行，仍可访问当前用户的文件和网络。运行不可信脚本需要额外的操作系统权限隔离，当前版本不提供这项保证。
...
另外，这个结构里**没有 `id` 字段**——读 `musicInfo.id` 的脚本拿到的是 `undefined`
（这是若干公开音源的已知 bug）。我们**同样不提供**，以保持行为一致，而不是替它们掩盖问题。
