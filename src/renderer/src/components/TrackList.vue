 ...
 * rails on one screen is what reads as "each page has its own scrollbar".
 * Extend the box to the edge and hand that 28 px back as right padding on each
 * of the three rows the list can render (head, track rows, and the selection
 * toolbar, whose ✕ would otherwise slide under the new rail), so the rail moves
 * and nothing that is read does. Measured: 时长 / ⋯ / 工具栏 keep their old x,
 * the rail goes 1252 → 1280.
 */
.view .tracklist{margin-right:calc(-1 * var(--view-pad-x, 28px))}
.view .track-head,.view .track-row,.view .selection-toolbar{padding-right:calc(12px + var(--view-pad-x, 28px))}
</style>
