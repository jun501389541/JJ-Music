...
**进程边界不等于权限沙箱**。脚本在 Node 全局上下文执行，仍可访问当前用户的文件和网络。
真实案例在 1.2.1。
...
1. **能力剥夺**（`source-host.ts` 的 `lockDownProcess`）
...
1.2.1 的三层防御有个共同短板：**它们都在脚本已经跑起来之后才起作用**。而用户提出的
真实场景是「不小心误触开启音源」——开关是一次点击，等到隔离生效时，脚本已经执行过了。
