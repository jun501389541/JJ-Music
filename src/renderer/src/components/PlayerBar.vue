...
*/
.playbar{position:relative;display:flex;align-items:center;gap:15px;height:var(--playbar-height);flex:none;background:var(--bg-elevated);padding:10px 24px;border-top:1px solid var(--divider)}
.mini-progress{position:absolute;top:-9px;left:0;right:0;opacity:0;transition:opacity .2s}
.playbar:hover .mini-progress,.mini-progress:focus-within{opacity:1}
.mini-side{display:flex;align-items:center;gap:10px;flex:1 1 0;min-width:0}
.mini-side--right{justify-content:flex-end}
...
.mini-lyric{min-width:0;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;background:none;border:0;color:var(--text-secondary);font:inherit;font-size:12px;cursor:pointer;text-align:right}
.mini-right{display:flex;align-items:center;gap:10px;flex:none}
.output-spec{display:flex;align-items:center;flex-direction:column;gap:4px;min-width:55px;color:var(--text-secondary)}
