...
export interface SettingItem {
  ...
  key?: keyof AppSettings; kind?: 'toggle' | 'select' | 'range' | 'color' | 'info'
...
const select = (label: string, key: keyof AppSettings, options: Array<[string, string | number]>): SettingItem => ({ label, key, kind: 'select', options: options.map(([label, value]) => ({ label, value })) })
...
export const SETTINGS_PAGES: Record<string, SettingsPage> = {
  ...
  data: { title: 'App 数据', icon: 'folder', items: [{ label: '本地保存', kind: 'info', description: '设置、歌单、音源与曲库索引保存在当前用户的应用数据目录。音乐文件保留在原位置。' }] },
