...
| 发现音乐 | 最近播放、曲库概览 |
| 全局搜索 | 本地结果支持拼音全拼与首字母，输入时有联想提示（顶栏的放大镜与搜索页同一套）；未输入时显示搜索历史与热门搜索词，两者可在设置里各开关；在线可搜 QQ、网易云、酷狗、酷我、咪咕 |
...
- 热门搜索词只有 QQ 与网易云提供可公开访问的端点（本机实测）：酷狗的搜索主机连不上、酷我的热词接口返回 0 条、咪咕返回「路由请求不支持」，这三家的页签里那一行直接不显示，不会转圈占位。QQ 的主机偶发连接超时，此时该行同样不出现，切页签会再问一次。
